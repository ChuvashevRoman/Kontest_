...
len_h1 = 6 # Количество нейронов в первом слое
len_h2 = 6 # Количество нейронов во втором слое
len_output = 1
    ...
    for n in range(len(x)):
        ...
        for i in range(len(input_i[0])):
            for j in range(len(input_i[0][i])):
                ...
                # Обратный ход
                sigma_3 = (y[0][i][j] - o_1) * sigmoid_der(t_3)
                d_w3 = h_2.T @ sigma_3
        ...
        mse = mse_loss(y_pred, y[0])
        print("==============================================")
        print(f"Ошибка на шаге {n}: {mse}")
    if step > steps:
        print("*********************************")
        print(f"УШЛА ЭПОХА {step}")
        print(f"ПРИШЛА ЭПОХА {step - 1}")
...
y_predict = []
for item in x:
    y_pred_i = feedforwarf(item, w, b)
    y_predict.append(y_pred_i)
